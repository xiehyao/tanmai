...
def _upload_bytes_to_cos(
    file_bytes: bytes, filename: str, content_type: str, key_prefix: str
) -> dict:
    ...
    key = f"{prefix}/{yyyymm}/{uuid.uuid4().hex}{ext}"
...
def upload_post_image_to_cos(file_bytes: bytes, filename: str, content_type: str) -> dict:
    return _upload_bytes_to_cos(file_bytes, filename, content_type, "posts")


def upload_card_personal_photo_to_cos(file_bytes: bytes, filename: str, content_type: str) -> dict:
    """名片编辑页「个人相片」上传到 COS，与动态图片分前缀存放。"""
    return _upload_bytes_to_cos(file_bytes, filename, content_type, "cards/personal_photos")
...
def upload_bytes_to_cos(
    ...
    """上传任意二进制到 COS，用于混元结果图等（与历史 API 兼容）。"""
    return _upload_bytes_to_cos(file_bytes, "", content_type, key_prefix)
