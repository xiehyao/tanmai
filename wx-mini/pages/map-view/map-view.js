// pages/map-view/map-view.js（非 tab 页面，可返回）
...
Page({
  ...
  onLoad(options) {
    const userId = options && options.user_id
    const lat = options && options.lat ? parseFloat(options.lat) : null
    const lng = options && options.lng ? parseFloat(options.lng) : null
    if (userId && lat != null && lng != null && !isNaN(lat) && !isNaN(lng)) {
      this.setData({ centerLat: lat, centerLng: lng, scale: 14 })
      this.loadNearbyFriends(lat, lng)
      this.loadCardData(userId)
    } else {
  ...
  async updateMarkers() {
    ...
    Object.keys(groups).forEach((key) => {
      ...
      group.forEach((friend, index) => {
        ...
        flatList.push({ friend, key, index, latitude, longitude, displayName })
      })
    })
    const avatarCache = {}
